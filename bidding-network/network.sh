...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/tn/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for TnOrg and TcOrg"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/tn/msp organizations/fabric-ca/tn/tls-cert.pem organizations/fabric-ca/tn/ca-cert.pem organizations/fabric-ca/tn/IssuerPublicKey organizations/fabric-ca/tn/IssuerRevocationPublicKey organizations/fabric-ca/tn/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/tc/msp organizations/fabric-ca/tc/tls-cert.pem organizations/fabric-ca/tc/ca-cert.pem organizations/fabric-ca/tc/IssuerPublicKey organizations/fabric-ca/tc/IssuerRevocationPublicKey organizations/fabric-ca/tc/fabric-ca-server.db'
...
CLI_DELAY=3
# channel name defaults to "bidding-channel"
CHANNEL_NAME="bidding-channel"
